...
def plot_box(topic_scores_df, lda_scores_df, image_saved_dir):
    ...
    for (metric, topic_num), group in plot_df.groupby(["metric", "topic_num"]):
        ...
        for name in plot_rows:
            ...
            fig.add_trace(go.Box(y=scores, name=name))
            topic_scores_all[metric][name].extend(scores)
        lda_rows = lda_scores_df[(lda_scores_df["metric"] == metric) & (lda_scores_df["topic_num"] == topic_num)]
        lda_score = np.stack(lda_rows["scores"].values).flatten()
        lda_scores_all[metric].extend(lda_score)
        fig.add_trace(go.Box(y=lda_score, name="LDA"))
        fig.update_layout(title=f"Box plot of {metric} scores for {topic_num} topics", template="plotly_white",
                          width=1000, height=500, yaxis_title=f"{metric} score")
        os.makedirs(image_saved_dir / task, exist_ok=True)
        fig.write_image(image_saved_dir / task / f"{metric}_{topic_num}.png")
    for metric, scores in topic_scores_all.items():
        fig = go.Figure()
        for name, score in scores.items():
            fig.add_trace(go.Box(y=score, name=name))
        fig.add_trace(go.Box(y=lda_scores_all[metric], name="LDA"))
        fig.update_layout(title=f"Box plot of {metric} scores for all topics", template="plotly_white",
                          width=1000, height=500, yaxis_title=f"{metric} score")
