...
def train(config_parser: ConfigParser):
    logger = config_parser.get_logger("MIND RS training")
    # setup data_loader instances
    data_loader = config_parser.init_obj("data_config", module_data, True)
    # build model architecture, then print to console
    model = config_parser.init_obj("arch_config", module_arch, True)
    logger.info(model)

    # get function handles of loss and metrics
    trainer = MindRSTrainer(model, config_parser, data_loader)
    trainer.train()


def train_dist(local_rank, nprocs, config_parser):
    # TODO: Distributed Training
    logger = config_parser.get_logger("mind train")
    ...
    bs = int(config_parser["data_loader"]["args"]["batch_size"] / nprocs)
    # setup data_loader instances
    data_loader = config_parser.init_obj("data_config", module_data, batch_size=bs)

    # build model architecture, then print to console
    model = config_parser.init_obj("arch", module_arch).to(local_rank)
    logger.info(model)
