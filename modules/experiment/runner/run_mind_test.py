...
from modules.utils import Tokenizer, init_model_class, load_batch_data, get_news_embeds
...
if __name__ == "__main__":
    ...
    saved_filename = config.get("saved_filename", "prediction")  # tag of evaluated model
    ...
    news_set, user_set = module_dataset.NewsDataset(test_set), module_dataset.UserDataset(test_set)
    ...
    with torch.no_grad():
        ...
        for batch_dict in tqdm(test_loader, total=len(test_loader)):  # run model
            batch_dict = load_batch_data(batch_dict, trainer.device)
            pred = trainer.model(batch_dict).cpu().numpy()
    ...
    with open(Path(saved_dir, saved_name, f"{saved_filename}.txt"), 'w') as f:  # saved predictions as txt file
        for impr_index, preds in tqdm(result_dict.items()):
            impr_index += 1
