...
class NewsBehaviorSet:
    def __init__(self, **kwargs):
        ...
        self.train_strategy = kwargs.get(
            "train_strategy", "pair_wise"
        )  # pair wise uses negative sampling strategy
        self.dataset_name = kwargs.get("dataset_name", "MIND")  # dataset name
        self.subset_dir = get_subset_dir(
            **kwargs
        )  # get directory of MIND dataset that stores news and behaviors
        self.subset_type = kwargs.get("subset_type", "small")
        self.data_root = Path(
            kwargs.get("data_dir", os.path.join(get_project_root(), "dataset"))
        )  # root directory
        self._init_news_features(**kwargs)
        self._load_category(cat_type="category", **kwargs)  # load category information
        self._load_category(
            cat_type="subvert", **kwargs
        )  # load subcategory information
    ...
    def _load_behaviors(self, **kwargs):
        ...
        default_uid_path = os.path.join(
            self.data_root, "utils", self.dataset_name, f"uid_{self.subset_type}.json"
        )
        ...
        with open(behaviors_file, "r", encoding="utf-8") as rd:
            ...
            for behavior in rd:
                ...
                if (
                    self.phase != "final_test"
                ):  # load labels for train and validation phase
                ...
                if self.phase == "train":  # negative sampling for training
                    pos_news = [
                        (news, index)
                        for label, news in zip(labels, candidate_news)
                        if label
                    ]
    ...
    def _load_category(self, cat_type, **kwargs):
        ...
        if getattr(self, f"use_{cat_type}") and cat_type in self.news_df.columns:
            setattr(
                self,
                f"{cat_type}2id",
                load_category(category_set=self.news_df[cat_type].unique(), **kwargs),
            )
            self.append_feature(cat_type)
            categories = self.news_features[cat_type]
            self.feature_matrix[cat_type] = np.stack(
                [self.convert_cat(c, cat_type) for c in categories]
            )
    ...
    def _init_news_features(self, **kwargs):
        self.news_info = get_news_info(
            **kwargs
        )  # limited in ["title", "abstract", "body", "use_all"]
        ...
        self.news_attr = {
            k: length for k, length in zip(self.news_info, self.news_lengths)
        }
        default_nid_path = os.path.join(
            self.data_root, f"utils/{self.dataset_name}/nid_{self.subset_type}.json"
        )
        nid_path = Path(
            kwargs.get("nid_path", default_nid_path)
        )  # get nid path from kwargs
        self.news_df = pd.read_csv(
            self.data_root / f"{self.dataset_name}/{self.subset_type}/news.csv"
        ).fillna("")
        if nid_path.exists():
            self.nid2index = read_json(nid_path)
        else:
            self.nid2index = dict(
                zip(self.news_df.news_id, range(1, len(self.news_df) + 1))
            )
            os.makedirs(nid_path.parent, exist_ok=True)
            write_json(self.nid2index, nid_path)
        self.news_df["index"] = self.news_df["news_id"].apply(
            lambda x: self.nid2index[x]
        )
        self.news_features = OrderedDict(
            {k: [""] * (len(self.nid2index) + 1) for k in self.news_info}
        )
        for attr in self.news_info:
            if attr == "use_all":
                # use all information of news (title, abstract, body)
                self.news_df["use_all"] = (
                    self.news_df["title"]
                    + " "
        ...
        self.feature_matrix = OrderedDict(
            {  # init news text matrix
                k: np.stack(
                    self.tokenizer.tokenize(
                        news_text, self.news_attr[k], return_tensors=False
                    )
                )
