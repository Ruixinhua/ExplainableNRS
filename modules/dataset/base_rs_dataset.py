...
class NewsRecDataset(Dataset):
    ...
    def __init__(self, tokenizer, **kwargs):
        ...
        self.neg_pos_ratio = kwargs.get(
            "neg_pos_ratio", 4
        )  # negative sampling ratio, default is 20% positive ratio
        self.load_object = kwargs.get("load_object", False)
        default_object_path = (
            Path(self.data_root)
            / f"utils/{self.dataset_name}/{self.subset_type}_{self.phase}.pt"
        )
    ...
    def __getitem__(self, index):
        ...
        user_index, history = (
            self.behaviors["uid"][imp_index],
            self.behaviors["history_news"][imp_index],
        )
        if self.train_strategy == "pair_wise":  # negative sampling
            labels, candidate_news = (
                self.behaviors["labels"][imp_index],
                self.behaviors["candidate_news"][imp_index],
            )
        ...
        input_feat = {
            "history_length": torch.tensor(
                self.behaviors["history_length"][imp_index], dtype=torch.int32
            ),
            "label": torch.tensor(label, dtype=torch.long),
            "uid": torch.tensor(user_index, dtype=torch.long),
        }
    ...
    def load_news_index(self, indices, input_name):
        ...
        :param input_name: corresponding saved name of feat, candidate or history
...
class NewsDataset(Dataset):
    ...
    def __len__(self):
        return len(
            self.dataset.feature_matrix[list(self.dataset.feature_matrix.keys())[0]]
        )
...
class ImpressionDataset(Dataset):
    def __init__(self, dataset: NewsRecDataset, news_embeds=None, selected_imp=None):
        ...
        if selected_imp:
            ...
            indices = [
                imp_indices.index(int(i)) for i in selected_imp if i in imp_indices
            ]
            self.behaviors = {
                k: [v[i] for i in indices] for k, v in self.behaviors.items()
            }
    ...
    def __getitem__(self, index):
        ...
        input_feat = {
            "impression_index": torch.tensor(
                self.behaviors["impression_index"][index], dtype=torch.int32
            ),  # index
            "candidate_length": torch.tensor(
                len(candidate), dtype=torch.int32
            ),  # length of candidate news
            "history_length": torch.tensor(
                self.behaviors["history_length"][index], dtype=torch.int32
            ),
            "uid": torch.tensor(self.behaviors["uid"][index], dtype=torch.long),
        }
        if self.dataset.phase != "final_test":
            input_feat.update(
                {"label": torch.tensor(self.behaviors["labels"][index])}
            )  # load true label of behaviors
        if self.news_embeds is not None:
            input_feat["candidate_news"] = torch.tensor(
                self.news_embeds[candidate]
            )  # load news embed from cache
            input_feat["history_news"] = torch.tensor(
                self.news_embeds[history]
            )  # load news embed from cache
        input_feat.update(
            self.dataset.load_news_index(candidate, "candidate")
        )  # load candidate news input
        input_feat.update(
            self.dataset.load_news_index(history, "history")
        )  # load candidate news input
        return input_feat
