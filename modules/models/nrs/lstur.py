...
class LSTURRSModel(MindNRSBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.masking_probability = 1.0 - kwargs.get(
            "long_term_masking_probability", 0.1
        )
        self.use_category, self.num_filters = kwargs.get(
            "use_category", False
        ), kwargs.get("num_filters", 300)
        ...
        self.news_encode_layer = Conv1D(
            self.embedding_dim, self.num_filters, self.window_size
        )
        self.news_att_layer = AttLayer(
            self.num_filters, self.attention_hidden_dim
        )  # output size is [N, num_filters]
        ...
        if self.user_embed_method == "init" or self.user_embed_method == "concat":
            ...
            self.user_embedding = nn.Embedding(
                len(uid2index) + 1, user_dim
            )  # count from 1
            self.user_affine = nn.Linear(user_dim, news_dim)
        self.user_encode_layer = nn.GRU(
            news_dim, news_dim, batch_first=True, bidirectional=False
        )
    ...
    def news_encoder(self, input_feat):
        ...
        if self.use_category:
            ...
            vector = torch.cat(
                [vector, self.dropouts(category_vector), self.dropouts(subvert_vector)],
                dim=1,
            )
    ...
    def user_encoder(self, input_feat):
        ...
        if self.user_embed_method == "init":
            user_embed = F.relu(
                self.user_affine(self.user_embedding(user_ids)), inplace=True
            )
